...
def draw_overlay(frame, detection, dt) -> None:
    ...
    if detection.found:
        if KF is None:
            KF = KalmanFilter(initial_x=detection.x, initial_y=detection.y, accel_variance=250000.0, g=9.81, R=R)  # Initialize Kalman Filter with gravit
        ...
        print(f"pos=({predicted_state[0]},{predicted_state[1]}) vel=({predicted_state[2]},{predicted_state[3]}) dt={dt*1000:}ms")
        predicted_state = predicted_state.flatten()  # Flatten the state for easier access
        cv2.circle(frame, (int(predicted_state[0]), int(predicted_state[1])), 8, (255, 0, 0), -1)  # Draw predicted position
        ...
        print(f"pos=({update_state[0]},{update_state[1]}) vel=({update_state[2]},{update_state[3]}) dt={dt*1000:.0f}ms")

        cv2.circle(frame, (int(update_state[0]), int(update_state[1])), 8, (0, 0, 255), -1)  # Draw updated position
        ...
        if KF is not None:
            ...
            predicted_state = predicted_state.flatten()  # Flatten the state for easier access
            cv2.circle(frame, (int(predicted_state[0]), int(predicted_state[1])), 8, (255, 0, 0), -1)  # Draw predicted position
        cv2.putText(frame, "No orange detected", (10, 25),
